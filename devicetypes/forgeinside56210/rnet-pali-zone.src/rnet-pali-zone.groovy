metadata {
	definition (name: "RNET Pali Zone", namespace: "forgeinside56210", author: "PaliK", mnmn: "SmartThingsCommunity", vid: "655676fd-c4c7-34b9-8476-6bb6b59ac761"){
    ...
    capability "forgeinside56210.rnetSwitch"
...
def setVolume(value) { 
        sendCommand(["volume": (value/2).intValue()], true)
        device.setVolume(value)
    }

def setRnetSwitch(value) { 
	if(value) { 
    	on()
        setRnetSwitch.on()
    } else { 
...
def setRnetInput(value) { 
	
    //log.debug "Source value is set to: " + value
   
    switch(value) {
        case "S1":
        sendCommand(["source": 0], true);
        device.setRnetInput(value)
        break;
        ...
        case "S3":
        sendCommand(["source": 2], true);
        device.setRnetInput(value)
        break;

        case "S4":
        sendCommand(["source": 3], true);
        device.setRnetInput(value)
        break;

        case "S5":
        sendCommand(["source": 4], true);
        device.setRnetInput(value)
        break;
...
def allOn() { sendCommand(["all": 1], false) }
...
def zone(evt) {
  log.debug "PALI DEBUG - ${evt}"
  log.debug "PALI DEBUG - ZONE${getZone()} zone(${evt})"
  ...
  */
  if (evt.containsKey("state")) {
    ...
    //turn off party mode
    /* Pali turned off all party mode code
...
def partyMode(evt) {
  ...
  if (evt.containsKey("state")) {
    ...
    // Pali turned off all party mode code sendEvent(name: "partyMode", value: (evt.state == 1) ? "on" : "off")
